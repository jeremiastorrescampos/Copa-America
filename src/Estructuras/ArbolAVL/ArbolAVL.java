...
public class ArbolAVL {
    ...
    public boolean insertar(Comparable elemento){
        ...
        if (this.raiz == null){
            ...
            if (exito){/* Si se insertó correctamente, se realiza el balanceo del árbol     */
                this.raiz = realizarBalanceAvl(this.raiz); // Se actualiza la raíz del árbol después de realizar el balanceo, ya que la raíz podría haber cambiado debido a las rotaciones realizadas durante el proceso de balanceo.
    ...
    private boolean insertarAux(NodoAVL n, Comparable elemento){
        ...
        if ((elemento.compareTo(n.getElem()) == 0)){
            exito = false;//manejo de duplicados, no se permite insertar un elemento que ya existe en el árbol.
        }else{
            if (elemento.compareTo(n.getElem()) < 0){/* Si el elemento es menor que el nodo actual */
    ...
    private NodoAVL realizarBalanceAvl(NodoAVL n){
        n.recalcularAltura();// Se recalcula la altura del nodo antes de verificar el balance, ya que puede haber cambiado debido a la inserción o eliminación de un nodo hijo. Esto asegura que el balance se evalúe correctamente en función de la altura actualizada del nodo.
        NodoAVL nuevaRaiz = n;
    ...
    /* Se obtiene el balance del nodo con sus hijos */
    ...
    private void borrarNodo(NodoAVL n){//Siempre busco al meyor de los menores, es decir el nodo mas a la derecha del subarbol izquierdo del nodo a eliminar
    ...
    private Comparable  buscarCandidato(NodoAVL n){
        ...
        if (n != null){
            if (n.getDerecho().getDerecho() == null){
                if(n.getDerecho().getIzquierdo() != null){// Si el nodo candidato tiene un hijo izquierdo, se asigna ese hijo al nodo derecho del nodo actual, ya que el nodo candidato se va a eliminar y su
                    candidato = n.getDerecho().getElem();
                    n.setDerecho(n.getDerecho().getIzquierdo());
                }else{
                    candidato = n.getDerecho().getElem();
                    n.setDerecho(null); // Si el nodo candidato no tiene hijos, simplemente se elimina el nodo derecho del nodo actual. 
                }
            }else{
